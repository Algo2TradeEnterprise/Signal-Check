...
Public Class PiercingAndDarkCloud
    Inherits Rule
                            ...
                            For Each runningPayload In currentDayPayload.Keys
                                ...
                                If currentDayPayload(runningPayload).PreviousCandlePayload IsNot Nothing Then
                                    If currentDayPayload(runningPayload).CandleColor = Color.Green Then
                                        ...
                                        currentDayPayload(runningPayload).Close >= currentDayPayload(runningPayload).PreviousCandlePayload.High - currentDayPayload(runningPayload).PreviousCandlePayload.CandleRange / 2 Then
                                            ...
                                            ret.Rows.Add(row)
                                        End If
                                    ElseIf currentDayPayload(runningPayload).CandleColor = Color.Red Then
                                        If currentDayPayload(runningPayload).Open > currentDayPayload(runningPayload).PreviousCandlePayload.High AndAlso
                                        currentDayPayload(runningPayload).Low >= currentDayPayload(runningPayload).PreviousCandlePayload.Low AndAlso
                                        currentDayPayload(runningPayload).Close <= currentDayPayload(runningPayload).PreviousCandlePayload.High - currentDayPayload(runningPayload).PreviousCandlePayload.CandleRange / 2 Then
                                            Dim row As DataRow = ret.NewRow
                                            row("Date") = runningPayload
                                            row("Instrument") = currentDayPayload(runningPayload).TradingSymbol
