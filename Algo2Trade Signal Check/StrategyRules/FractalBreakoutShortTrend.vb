...
Public Class FractalBreakoutShortTrend
    Inherits Rule
                ...
                For Each stock In stockList
                    ...
                    If stockPayload IsNot Nothing AndAlso stockPayload.Count > 0 Then
                        ...
                        If currentDayPayload IsNot Nothing AndAlso currentDayPayload.Count > 0 Then
                            Dim outsideFractalSignal As Boolean = False
                            Dim returnInsideFractal As Boolean = False
                            Dim fractalLevel As Decimal = 0
                            For Each runningPayload In currentDayPayload.Keys
                                _canceller.Token.ThrowIfCancellationRequested()
                                If currentDayPayload(runningPayload).Low > fractalHighPayload(runningPayload) Then
                                    outsideFractalSignal = True
                                ElseIf currentDayPayload(runningPayload).High < fractalLowPayload(runningPayload) Then
                                    outsideFractalSignal = False
                                    returnInsideFractal = False
                                    fractalLevel = 0
                                End If
                                If outsideFractalSignal AndAlso currentDayPayload(runningPayload).Low < fractalHighPayload(runningPayload) Then
                                    returnInsideFractal = True
                                End If
                                If outsideFractalSignal AndAlso returnInsideFractal AndAlso
                                    fractalLowPayload(runningPayload) > fractalLowPayload(currentDayPayload(runningPayload).PreviousCandlePayload.PayloadDate) Then
                                    fractalLevel = fractalLowPayload(runningPayload)
                                    outsideFractalSignal = False
                                    returnInsideFractal = False
                                ElseIf outsideFractalSignal AndAlso Not returnInsideFractal AndAlso
                                    fractalLowPayload(runningPayload) > fractalLowPayload(currentDayPayload(runningPayload).PreviousCandlePayload.PayloadDate) Then
                                    outsideFractalSignal = False
                                End If
                                If fractalLevel <> 0 AndAlso fractalLevel <> fractalLowPayload(runningPayload) Then
                                    fractalLevel = 0
                                ElseIf fractalLevel <> 0 AndAlso currentDayPayload(runningPayload).Low < fractalLevel Then
